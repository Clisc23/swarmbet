...
/**
 * Handle the full anonymous vote flow: ensure election exists, then cast vote.
...
export async function handleAnonymousVote(
  ...
  const electionId = await ensureVocdoniElection(poll, web3auth.provider);

  toast.info('Generating ZK proof...', { duration: 5000 });
  const voteId = await castAnonymousVote(web3auth.provider, electionId, optionIndex);
  return voteId;
}
